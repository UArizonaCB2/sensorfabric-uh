...
# Enhanced build and deploy script for Ultrahuman SensorFabric Lambda functions
...
setup_build_directories() {
    ...
    for local_func in "${!LAMBDA_FUNCTIONS[@]}"; do
        ...
        cp -r ultrahuman/ "$BUILD_DIR/$local_func/"
        ...
        sed -i "s/CMD \[\".*\"\]/CMD [\"ultrahuman.${local_func}.lambda_handler\"]/" "$BUILD_DIR/$local_func/Dockerfile"
...
main() {
    print_header "Starting Ultrahuman SensorFabric Lambda build and deploy process..."
